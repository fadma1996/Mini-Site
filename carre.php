...
<form method="POST">
             Coté (
			<td><input type="text" name="cote" />)2</td>
			<td><input type="submit" name="egual" value="="/></td>
			<?php
			if(!empty($_POST['cote']))
			{
				echo "<input type=\"text\" value=\"".round($_POST['cote']*$_POST['cote'])."\"/>";
			}
			?>
			

<p>Indiquez la longueur d'un coté pour avoir l'aire du carré.</p>
<h2>Comment calculer l'aire d'un carré</h2>
<p>Pour calculer l'aire d'un carré, on multiplie la longueur de l'un de ses cotés par lui-meme.<p>
<h3>Aire du carré = Coté X Coté</h3>
<h4>Exemple</h4>
<p>Soit un carré ABCD avec AB= 4 cm<br><img src="images/carree.jpg" align="right"><br>
L'aire du carré ABCD = 4cm X 4cm<br>
L'aire du carré ABCD = 16 cm2</p>
<h2>Autre formule pour calculer l'aire d'un carré</h2>
<p>On peut calculer de l'aire d'un carré en connaissant la longueur de sa diagonale.</p>
<h3>Aire du carré =(Diagonale)2/2</h3>
</form>
</div>
</body>
</html>